 ...
 * @authorLink https://github.com/One-Man-Dev1/
 * @source https://github.com/One-Man-Dev1/priveta-numero-1/blob/main/j.js
 * @updateUrl https://raw.githubusercontent.com/One-Man-Dev1/priveta-numero-1/main/j.js
 */
	...
	const config = {
		...
		},
		{
			...
			settings: [
				...
				{ id: "test", name: "test", type: "switch", value: true }
			]
		}],
		/* Change Log */
		changelog: [{
			...
			const plugin = (Plugin, Api) => {
				try {
					...
					const sounds = [
					    ...
					    {re: /test/gmi, file: "boowomp.mp3", duration: 930}
					];
							...
							if (!optimistic && lastMessageID != message.id) {
								...
								for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
									if (this.settings.toggle[sound[1].file.replace(/\..+$/, "")]) {
										let audio = new Audio("https://github.com/One-Man-Dev1/priveta-numero-1/tree/main/sounds" + sound[1].file);
